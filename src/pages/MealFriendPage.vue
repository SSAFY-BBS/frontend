<template>
  <div class="max-w-4xl mx-auto py-10 px-4 relative">
    <div class="text-center mb-10">
      ...
      <p class="text-slate-500">매칭이 되면 저는 반드시 같이 점심을 먹을 것을 엄숙히 선서합니다.</p>
    </div>
    ...
    >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        ...
        >
          ...
          <template v-else-if="!isRevealed">
            <svg
              class="w-10 h-10 text-sky-500 mb-2"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <template v-else>
            <div
              class="w-12 h-12 rounded-full bg-white shadow-sm flex items-center justify-center mb-3 text-xl"
            >
              🍽️
            </div>
            <span class="text-xs font-bold text-sky-600 mb-1">{{
              participants[index - 1]?.group
            }}</span>
      ...
      <div class="text-center" v-else-if="!isRevealed">
        <p class="text-slate-600 font-medium animate-pulse">
          {{
            isMatching
              ? '매칭이 완료되었습니다! 결과를 확인합니다...'
              : `다른 친구들을 기다리고 있습니다... (${participants.length}/4)`
          }}
        </p>
      </div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center px-4">
      ...
      <div class="bg-white w-full max-w-sm rounded-2xl p-6 shadow-xl relative z-10">
        ...
        <div class="flex gap-2">
          <button
            @click="isModalOpen = false"
            class="flex-1 bg-slate-100 text-slate-600 py-2 rounded-lg font-medium hover:bg-slate-200"
          >
            취소
          </button>
          <button
            @click="submitNote"
            class="flex-1 bg-sky-500 text-white py-2 rounded-lg font-medium hover:bg-sky-600"
          >
            등록 완료
          </button>
        </div>
...
const { participants, isMatching, isFlashing, isRevealed, submitNoteAPI, resetMatchState } =
  useMealFriendWS()

const getSlotState = (index: number) => {
  const isFilled = !!participants.value[index - 1]
  if (!isFilled)
    return { bgClass: 'bg-white', borderClass: 'border-2 border-dashed border-slate-200' }
  if (!isRevealed.value)
    return { bgClass: 'bg-sky-50', borderClass: 'border-2 border-solid border-sky-200' }
...
/* 기존 애니메이션 CSS 유지 */
.drumroll-active {
  animation: drumroll 4s cubic-bezier(0.4, 0, 0.2, 1) forwards;
}
@keyframes drumroll {
  0% {
    transform: scale(1);
  ...
  96% {
    ...
    box-shadow: 0 0 60px rgba(14, 165, 233, 0.9);
  }
  100% {
    transform: scale(1.07) translate(0, 0);
    filter: brightness(0.5);
    box-shadow: 0 0 80px rgba(14, 165, 233, 1);
  }
}
